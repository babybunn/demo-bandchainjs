...
export default function ModalGenerateWallet() {
  const { isShowCreateWallet, setIsShowCreateWallet } = useContext(ModalContext);
  const closeModalHandler = (event) => setIsShowCreateWallet(false);
  const wallet = useSelector((state) => state.wallet);

  useEffect(() => {
    setIsShowCreateWallet(false);
  }, [wallet, setIsShowCreateWallet]);
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div className="mt-2">
              <p className="text-center">
                ...
                <span className="text-blue underline font-medium cursor-pointer">
                  Connect wallet
                </span>
              </p>
            </div>
